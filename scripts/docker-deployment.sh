...
Help()
{
    ...
    echo ""
    echo "**********************************************************************************************************************"
    echo "HELP: UBKG neo4j Docker container deployment script"
    echo
    echo "Syntax: $0 -c <config file path>"
    echo "-c   path to configuration file containing parameters for running the container (REQUIRED: default=container.cfg)"
    echo "Note the external bind mount point for neo4j database files specified as a configuration parameter must already exist."
...
echo "Checking for external bind mount volume directory at db_mount_dir=$db_mount_dir"
if [ ! -d "$db_mount_dir" ]
then
    echo "Error: no directory exists at configured external bind mount path '$db_mount_dir'."
    echo "A full set of Neo4j 5.x database files must exist at '$db_mount_dir/databases/neo4j'"
    exit 1
fi

ont_db_dir="$db_mount_dir"/databases/neo4j
echo "Checking for the ontology database directory at ont_db_dir=$ont_db_dir"
if [ ! -d "$ont_db_dir" ]
then
    echo "Error: no Neo4j database directory exists at '$ont_db_dir'."
